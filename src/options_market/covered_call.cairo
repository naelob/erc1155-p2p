...
from src.options_market.interfaces.IBlackScholes import IBlackScholes
from src.options_market.interfaces.IExchangeAMM import IExchangeAMM
...
@event
func CallOptionBought(
    option_id : Uint256, caller : felt, premium : felt
){
}
...
// Contract Address of Echnage AMM from Realms Game
@storage_var
func exchange_amm_address() -> (res: felt) {
}

// Numbers of call option contracts i.e open interest
@storage_var
func options_counter() -> (res: Uint256) {
}
...
@storage_var
func min_above_bid_alpha() -> (res: Uint256) {
}
...
@storage_var
func assets_owned_by_this_contract(id : Uint256) -> (res: ResourceAsset) {
}
...
func initializer{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    options_counter.write(Uint256(1,0));
...
@view
func get_option_item{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    idx : Uint256
)-> (res : CallOption){
...
func write{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr} (
    ...
    let (settlement_token) = settlement_token_address.read();
    ...
    with_attr error_message("Covered_Call : ERC1155 and Settlement Token Must Be Different") {
        assert_not_equal(settlement_token, token_address);
    }
    ...
    let (local token_id_uint) = _uint_to_felt(token_id);

    assert [data] = caller;
    assert [data + 1] = token_address;
    assert [data + 2] = token_id_uint;
    ...
    let (NFT_AD) = nft_address.read();
    let (local null : Uint256*) = alloc();
    INftCoveredContract.mint(NFT_AD, caller, count, 0, null);

    let (next_count : Uint256, _) = uint256_add(count, Uint256(1,0));
...
func bid{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (caller) = get_caller_address();
    let (settlement_token) = settlement_token_address.read();
    let (balance_LORDS : Uint256) = IERC20.balanceOf(settlement_token, caller);

    let (LORDS_felt) = _uint_to_felt(balance_LORDS);

    with_attr error_message("Covered_Call : Not Enough Balance") {
        assert_lt(bid_amount, LORDS_felt);
    }
    ...
    //with_attr error_message("Covered_Call : Increment below MinAboveBidAlpha : Overbid Not Sufficicient") {
       ...
       // let (low : Uint256, high : Uint256) = uint256_mul(actual_bid, min_above_bid_alpha_);
       // let (res : Uint256, _) = uint256_unsigned_div_rem(Uint256(low,high), Uint256(10000,0));
        //let (final : Uint256, _) = uint256_add(res, actual_bid);
    ...
    let (local bid_uint : Uint256) = _felt_to_uint(bid_amount);
...
func buy{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    IERC20.transferFrom(settlement_token, caller, this_address, option_price_uint);

    // transfer this amount to writer of the option
    IERC20.transferFrom(settlement_token, this_address, option_info.Writer, option_price_uint);

    // Transfers the NFT representation of the option to the caller
    let (owner) = INftCoveredContract.ownerOf(NFT_AD, option_id);
    INftCoveredContract.transferFrom(NFT_AD, owner, caller, option_id);
...
func settle{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (settle_) = settle_type.read();
    if (settle_ == SettleType.Auction) { 
...
func reclaim_underlying{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    //burn the option NFT 
    INftCoveredContract.burn(NFT_AD, option_id);
...
func claim_option_earnings{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    alloc_locals;
    let (NFT_AD) = nft_address.read();
    let (owner_of_option_contract) = INftCoveredContract.ownerOf(NFT_AD, option_id);
    ...
    IERC20.transferFrom(settlement_token, contract, caller, claimable_uint);
...
func _set_new_asset_owner{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (local resource : ResourceAsset) = assets_owned_by_this_contract.read(asset_id);
    ...
    local new_resource : ResourceAsset =  ResourceAsset(
        new_owner,
...
func _settle_auction{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    IERC20.transferFrom(settlement_token, contract, option_info.Writer, option_info.Strike);
    let (caller) = get_caller_address();
    if (caller == owner_of_option_contract) {
        // send the spread to the option owner 
        IERC20.transferFrom(settlement_token, contract, owner_of_option_contract, spread);
        INftCoveredContract.burn(NFT_AD, option_id);
    ...
    let (spread_felt) = _uint_to_felt(spread);
    option_claims.write(option_id, spread_felt);
    CallOptionSettledByAuction.emit(option_id);
    return ();
...
func _settle_spot{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    with_attr error_message("Covered_Call : ERC1155 Resource Is Worth 0") {
        let (is_eq) = uint256_eq(spot_price_resource, Uint256(0,0)); 
        assert is_eq = 0;
    }
    ...
    //if (res == 1) {
        // send the strike price amount to the option writer
      //  IERC20.transferFrom(settlement_token, owner_of_option_contract, option_info.Writer, option_info.Strike); 
...
func _get_spot_price{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    alloc_locals;
    let (amm_address) = exchange_amm_address.read();
    // get prices from amm exchange for ERC1155 resource
    let (local assets : Uint256*) = alloc();
    assert [assets] = asset_id;
    let (local amounts : Uint256*) = alloc();
    assert [amounts] = Uint256(1,0);
    let (prices : Uint256*, _) = IExchangeAMM.get_all_rates(amm_address, 1, assets, 1, amounts);
    return (res=[prices]);
}
...
func _send_last_bid_to_previous_winner{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let bid : Uint256 = call_option.Bid;
    ...
    let (settlement_token) = settlement_token_address.read();
    let (this_address) = get_contract_address();
    IERC20.transferFrom(settlement_token, this_address, call_option.Winner, bid);
    return ();
}
...
func assert_bid_is_enabled{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (start_period) = auction_start_period.read();
    let diff =  option_info.Expiration - start_period;
