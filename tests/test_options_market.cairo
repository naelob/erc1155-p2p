...
func test_write_call_option{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    %{ stop_prank = start_prank(context.deployer_address,ids.covered_call_address) %}
...
func test_reclaim_underlying{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    // check NFT has been recieved by caller
    let (local balance_nft : Uint256) = INftCoveredContract.balanceOf(contract_address=nft_covered_contract,owner=caller);
    assert balance_nft.low = 1;
    assert balance_nft.high = 0;
    ...
    // check NFT recieved by caller is burned
    let (local balance_nft2 : Uint256) = INftCoveredContract.balanceOf(contract_address=nft_covered_contract,owner=caller);
    assert balance_nft2.low = 0;
    assert balance_nft2.high = 0;
    ...
    // check we received back our underlying

    let (local balance_final : Uint256) = IAssetErc1155.balanceOf(contract_address=erc1155_address,account=caller,id=Uint256(1,0));
    assert balance_final.low = 1;
    assert balance_final.high = 0;
